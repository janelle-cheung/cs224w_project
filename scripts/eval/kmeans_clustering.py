...
1. Per-patient files: {patient_id}_features.npy with separate labels dir
2. Split-based files: train/val/test_embeddings.npy with _labels.npy and _patient_ids.npy

Usage:
    python scripts/clustering_analysis.py --features_dir data/power64
    python scripts/clustering_analysis.py --features_dir data/graph_embeddings/gcn_k10_e64_t0.50_lr1e-03
"""
...
def load_split_embeddings(features_dir: Path) -> dict:
    """
    Load embeddings from split-based format (train/val/test_embeddings.npy).
    Returns dict mapping patient_id -> {'features': np.ndarray, 'labels': np.ndarray}.
    """
    all_features = []
    all_labels = []
    all_patient_ids = []
    ...
    for split in ['train', 'val', 'test']:
        emb_file = features_dir / f'{split}_embeddings.npy'
        labels_file = features_dir / f'{split}_labels.npy'
        patient_file = features_dir / f'{split}_patient_ids.npy'

        if emb_file.exists():
            all_features.append(np.load(emb_file))
...
def is_split_format(features_dir: Path) -> bool:
    """Check if directory uses split-based format."""
    return (features_dir / 'train_embeddings.npy').exists()


def analyze_patient_split(features: np.ndarray, labels: np.ndarray, k_range: range) -> dict:
    """
    Run clustering analysis for a single patient with pre-loaded labels (split format).
    Labels are 1D array of stage indices (0-4).
    """
    ...
    if len(results['k_values']) > 2:
        ...
        results['elbow_k'] = results['k_values'][0] if results['k_values'] else 1

    # Best k by BIC (minimum)
    if results['bics']:
        results['best_k_bic'] = results['k_values'][np.argmin(results['bics'])]
    else:
        results['best_k_bic'] = 1
...
def main():
    ...
    # Check if split-based format (train/val/test_embeddings.npy)
    if is_split_format(features_dir):
        print(f"Detected split-based format in {features_dir}")
        patient_data = load_split_embeddings(features_dir)
        ...
        for patient_id, data in sorted(patient_data.items()):
            ...
            results = analyze_patient_split(features, labels, k_range)
        ...
        # Original per-patient file format
    ...
    # Plot BIC, silhouette, and purity per patient
...
def plot_individual_metrics(all_results: dict, output_name: str):
    ...
    n_cols = 5
    n_rows = max(1, (n_patients + n_cols - 1) // n_cols)
